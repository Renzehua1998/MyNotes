...
- **GCL模型**是基于顶点集$V = {v_1, . . . , v_n}$ 上的随机图 $\mathcal G(\bold W, \mathcal{E}, α)$ 其中 vi, vj, 形成一条边的概率为：
  $$
...
在 G 上运行一些稳定的图聚类算法以获得顶点集 V 的分区 $\bold C$ ，一共产生 $l$ 个社区 $C_1, . . . C_l$。
...
给定 $α ∈ \mathbb R_+$，使用$\mathcal G(\bold W, \mathcal{E}, α)$的GCL 模型。
